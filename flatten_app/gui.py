...
class FlattenApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.tree = ttk.Treeview(tree_frame, columns=("type", "relpath", "count", "exclude"), selectmode="none", height=15, yscrollcommand=self.tree_scrollbar.set)
        ...
        self.tree.column("relpath", width=250)
        self.tree.column("count", width=80, anchor="center")
        self.tree.column("exclude", width=80, anchor="center")
        ...
        ext_frame = ttk.Frame(frm)
        ext_frame.grid(row=3, column=2, columnspan=2, sticky=tk.W+tk.E)
        ttk.Label(ext_frame, text="除外拡張子・ファイル名(1行1つ):").pack(anchor=tk.W)
        self.exclude_ext_text = tk.Text(ext_frame, height=4, width=28, font=('Meiryo UI', 10))
    ...
    def _flatten_thread(self, src, dst, zip_targets, exclude_targets, exclude_exts):
        try:
            ...
            for relpath in sorted(zip_targets):
                ...
                try:
                    # shutil.make_archiveは拡張子を自動で付与しないため、zip_pathは拡張子なしで渡す
                    shutil.make_archive(zip_path[:-4], 'zip', abs_dir)
                    # ただしfilemapにはzip_name（.zip付き）を記録
